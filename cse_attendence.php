...
if(isset($_POST['submit']))
{
    foreach($_POST['attendence_status'] as $id=>$attendence_status)
    {
       ...
       $roll = $_POST['roll'][$id];
       $mail = $_POST['email'][$id];
       mysqli_query($conn,"insert ignore into cse_attendence(roll_no,name,email,status) values('$roll','$name','$mail','$attendence_status')");
    }
}
...
<center>
    ...
    <br><br>
    <form action="cse_attendence.php" method="post">
        <table class="table">
            <thead>
            ...
            <tbody>
                <?php
                error_reporting(0);
$conn = mysqli_connect('localhost', 'root', '', 'attendance');                $result = mysqli_query($conn,"select * from student where dept_id=1");
                $serialnumber = 0;
                $counter= 0;
    ...
    <button><a href="cse_view_attendence.php" style="color:black; text-decoration:none;">View attendance</a></button>
</body>
</html>